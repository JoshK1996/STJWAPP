...
A dedicated employee, school, early-childhood and parish workspace. The current live milestone supports synthetic workforce demonstrations, employee timekeeping, organizational roles, reviewed time records, interactive workforce analytics and payroll-hours exports. The broader SIS/accounting scope remains unfinished.
...
Read [development and delivery](docs/DEVELOPMENT.md), [current status](docs/STATUS.md), [roadmap](docs/PLAN.md), [workforce analytics](docs/WORKFORCE-ANALYTICS.md), [payroll hours](docs/PAYROLL-HOURS.md) and [API](docs/API.md). The [local MCP bridge](integrations/mcp/README.md) exposes approved read-only API operations to compatible agents; it does not grant additional access.
