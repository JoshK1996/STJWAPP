...
async function inspect(bytes: Buffer, input: SnapshotXlsxInput) {
  ...
  assert.equal(names.filter(name => /^xl\/worksheets\/sheet\d+\.xml$/.test(name)).length, 3);
  ...
  for (const name of ["Data", "Provenance", "Source JSON"]) assert.ok(workbook.includes(`name="${name}"`));
  const data = cells(await zip.file("xl/worksheets/sheet1.xml")!.async("string"));
  for (const value of data.values()) assert.ok(["str", "inlineStr"].includes(value.type), value.type);
  const chunks = cells(await zip.file("xl/worksheets/sheet3.xml")!.async("string"));
  ...
  return { data, zip, provenance: cells(await zip.file("xl/worksheets/sheet2.xml")!.async("string")) };
}
...
test("actual empty-environment worker loads fixed ESM entry and emits bounded exact bytes", async () => {
  ...
  assert.equal(result.hash, hash(result.buffer)); assert.equal(result.bytes, result.buffer.length); assert.equal(result.formatVersion, 1);
