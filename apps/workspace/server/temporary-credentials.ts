...
const credentialDigest = (user: Row) => digest(JSON.stringify(["initial-credentials-v1", user.password_hash, user.pin_hash]));
...
export async function createStaffAccount(db: Database, supplied: Actor, sessionHash: string | undefined, raw: unknown, staffDomain: string, origin: string) {
  ...
  return db.transaction(async tx => {
    ...
    let result;
    if (hashes) {
      await tx.query("UPDATE users SET password_hash=$1,pin_hash=$2,requires_credential_change=true WHERE id=$3 AND org_id=$4", [...hashes, id, actor.org_id]);
      await audit(tx, actor, "staff.temporary_credentials_issued", id, { requiresCredentialChange: true });
      result = { id, requiresCredentialChange: true as const };
    } else {
...
export async function createCredentialChangeChallenge(tx: Queryable, user: Row) {
  ...
  return credentialChangeChallengeSchema.parse({ requiresCredentialChange: true, challenge, expiresAt });
}
...
export async function completeInitialCredentials(db: Database, raw: unknown) {
  ...
  const candidate = (await db.query(`SELECT u.id,u.org_id,u.active,u.requires_credential_change,u.password_hash,u.pin_hash,c.credential_digest
  ...
  const [samePassword, samePin] = await Promise.all([verifyPassword(input.password, candidate.password_hash), verifyPassword(input.pin, candidate.pin_hash)]);
  ...
  return db.transaction(async tx => {
    ...
    const pinLookup = await uniquePermanentPin(tx, user.id, input.pin);
    await tx.query("UPDATE users SET password_hash=$1,pin_hash=$2,requires_credential_change=false,pin_lookup=$4,pin_lookup_key_id=$5 WHERE id=$3", [passwordHash, pinHash, user.id, pinLookup.lookup, pinLookup.keyId]);
    ...
    await audit(tx, { id: user.id, org_id: user.org_id }, "auth.initial_credentials_replaced", user.id, { passwordReplaced: true, pinReplaced: true });
