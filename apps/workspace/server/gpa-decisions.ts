...
import { gpaPolicyEvidenceHash, decodeGpaPolicyVersionEvidence } from "./gpa-policies";
...
import { calculateTermGpa, TermGpaValidationError, termGpaCalculatorVersion } from "../shared/term-gpa";
import { renderGpaDecision } from "./gpa-decision-export";
import { checkedGpaReview, checkedGpaDecision, verifyGpaReviewData, deriveGpaSummary } from "./gpa-decision-evidence";
import { prepareGpaInput, retainGpaInput, eligibleGpaIssuesInput, gpaDecisionListInput, gpaDecisionCursorSchema,
  gpaDecisionExportInput, gpaPreviewSchema, gpaDecisionEnvelopeSchema, gpaDecisionSummarySchema,
  gpaDecisionListSchema, eligibleGpaIssuesSchema, gpaDecisionCurrentnessSchema,
  gpaDecisionLimits as limits, type GpaReviewData, type GpaPreview, type GpaDecisionSummary, type GpaDecisionDetail,
  type GpaDecisionList, type EligibleGpaIssues, type GpaDecisionCurrentness } from "../shared/gpa-decisions";

const iso = (value: any) => new Date(value).toISOString();
const unavailable = () => new Problem(404, "This gpa record is unavailable under your current access.");
const valid = (condition: unknown) => requireCondition(condition, 422, "The retained gpa evidence is inconsistent or unsupported.");
const changed = (condition: unknown) => requireCondition(condition, 409, "The reviewed gpa source or predecessor changed. Prepare and review it again.");
...
async function transaction<T>(db: Database, fn: (tx: Queryable) => Promise<T>, repeatable = false): Promise<T> {
  ...
  for (let attempt = 0; ; attempt++) {
    try { return await db.transaction(async tx => {
      ...
      if (["55P03", "57014", "40001", "40P01"].includes(error.code)) throw new Problem(503, "The gpa source is busy. Retry the same command when it is available.");
      if (error instanceof TermGpaValidationError) throw new Problem(422, "The confirmed policy does not apply to this selected reviewed source.");
      throw error;
    }
  }
}
...
async function policyVersion(tx: Queryable, orgId: string, unitId: string, yearId: string, versionId: string, lock = false): Promise<Row> {
  const row = (await tx.query(`SELECT v.*,p.unit_id,p.year_id,p.archived,p.active_policy_version_id FROM gpa_policy_versions v
    JOIN gpa_policies p ON p.id=v.policy_id AND p.org_id=v.org_id WHERE v.id=$1 AND v.org_id=$2 AND p.unit_id=$3 AND p.year_id=$4${lock ? " FOR SHARE OF p" : ""}`, [versionId, orgId, unitId, yearId])).rows[0];
  if (!row) throw unavailable();
  changed(!row.archived && row.active_policy_version_id === row.id);
  valid(gpaPolicyEvidenceHash(row.policy) === row.policy_hash);
  const policy = decodeGpaPolicyVersionEvidence(row).policy;
...
async function series(tx: Queryable, scope: Scope, lock: boolean): Promise<Row | undefined> {
  if (lock) await tx.query("SELECT pg_advisory_xact_lock(hashtextextended($1,0))", ["gpa-series:" + scopeParams(scope).join(":")]);
  return (await tx.query(`SELECT * FROM gpa_series WHERE org_id=$1 AND unit_id=$2 AND student_id=$3 AND year_id=$4 AND term_id=$5 AND policy_id=$6${lock ? " FOR UPDATE" : ""}`, scopeParams(scope))).rows[0];
}
function review(copy: StandingSourceCopy, version: Row, latestId: string | null, latestNumber: number): GpaReviewData {
  const normalized = normalizeStandingSource(copy), result = calculateTermGpa({ policy: version.policy, source: normalized.source });
  const confirmed = decodeGpaPolicyVersionEvidence(version);
  return verifyGpaReviewData({ schemaVersion: 1, calculatorVersion: termGpaCalculatorVersion,
    policyVersion: { id: version.id, policyId: version.policy_id, version: version.version, hash: version.policy_hash, configurationHash: version.configuration_hash }, policy: version.policy,
    policyConfirmation: { sourceDescription: confirmed.sourceDescription, reason: confirmed.reason, confirmedBy: confirmed.confirmedBy,
      confirmedAt: confirmed.confirmedAt, evidence: confirmed.evidence },
...
async function previewQuota(tx: Queryable, actor: Actor, bytes: number) {
  await tx.query("SELECT pg_advisory_xact_lock(hashtextextended($1,0))", ["gpa-previews-quota:" + actor.org_id]);
  await tx.query("DELETE FROM gpa_previews WHERE org_id=$1 AND (consumed_at IS NOT NULL OR expires_at<=clock_timestamp())", [actor.org_id]);
  const stats = (await tx.query(`SELECT count(*)::int AS count,COALESCE(sum(bytes),0)::text AS bytes,
    count(*) FILTER(WHERE prepared_by=$2)::int AS own FROM gpa_previews WHERE org_id=$1 AND consumed_at IS NULL AND expires_at>clock_timestamp()`, [actor.org_id, actor.id])).rows[0];
...
function summary(row: Row): GpaDecisionSummary {
  return gpaDecisionSummarySchema.parse({ id: row.id, seriesId: row.series_id, number: row.number, supersedesId: row.supersedes_id,
    policyVersionId: row.policy_version_id, outcome: row.outcome, gpa: row.gpa_summary, snapshotHash: row.snapshot_hash, jsonHash: row.json_hash, csvHash: row.csv_hash, capturedAt: iso(row.captured_at) });
}
export async function prepareGpa(db: Database, supplied: Actor, sessionHash: string | undefined, raw: unknown): Promise<GpaPreview> {
  const input = prepareGpaInput.parse(raw); requireCondition(sessionHash, 401, "A verified password session is required.");
  ...
  return transaction(db, async tx => {
    ...
    await tx.query(`INSERT INTO gpa_previews(id,org_id,unit_id,prepared_by,student_id,year_id,term_id,policy_id,policy_version_id,
    ...
    await audit(tx, actor, "gpa.preview.prepared", id, { unitId: scope.unitId, policyVersionId: version.id, previewHash, bytes, schemaVersion: 1 });
    await recheckReportSession(tx, actor, sessionHash);
    return gpaPreviewSchema.parse({ id, expiresAt, previewHash, data });
  });
}

export async function getGpaPreview(db: Database, supplied: Actor, sessionHash: string | undefined, rawId: unknown): Promise<GpaPreview> {
  requireCondition(sessionHash, 401, "A verified password session is required.");
  const id = z.uuid().parse(rawId);
  return transaction(db, async tx => {
    const row = (await tx.query("SELECT * FROM gpa_previews WHERE org_id=$1 AND prepared_by=$2 AND id=$3 AND consumed_at IS NULL AND expires_at>clock_timestamp()", [supplied.org_id, supplied.id, id])).rows[0];
    if (!row) { await currentReportActor(tx, supplied, sessionHash); throw unavailable(); }
    const actor = await currentOffice(tx, supplied, sessionHash, row.unit_id), data = checkedGpaReview(row.review_text, row.preview_hash);
    const guarded = (await tx.query("SELECT id FROM gpa_previews WHERE id=$1 AND org_id=$2 AND prepared_by=$3 FOR SHARE", [id, actor.org_id, actor.id])).rows[0];
    const live = guarded && (await tx.query("SELECT consumed_at IS NULL AND expires_at>clock_timestamp() AS live FROM gpa_previews WHERE id=$1", [id])).rows[0]?.live;
    if (!live) throw unavailable();
    await recheckReportSession(tx, actor, sessionHash!);
    return gpaPreviewSchema.parse({ id, expiresAt: iso(row.expires_at), previewHash: row.preview_hash, data });
  });
}

export async function retainGpa(db: Database, supplied: Actor, sessionHash: string | undefined, raw: unknown): Promise<{ decision: GpaDecisionSummary; replayed: boolean }> {
  const input = retainGpaInput.parse(raw); requireCondition(sessionHash, 401, "A verified password session is required.");
  ...
  return transaction(db, async tx => {
    ...
    await tx.query("SELECT pg_advisory_xact_lock(hashtextextended($1,0))", ["gpa-decision-command:" + supplied.org_id + ":" + supplied.id + ":" + input.commandId]);
    const receipt = (await tx.query("SELECT * FROM gpa_decision_commands WHERE org_id=$1 AND actor_id=$2 AND command_id=$3", [supplied.org_id, supplied.id, input.commandId])).rows[0];
    if (receipt) {
      const row = (await tx.query("SELECT *,encode(convert_to(csv_text,'UTF8'),'base64') AS csv_base64 FROM gpa_decisions WHERE org_id=$1 AND reviewed_by=$2 AND id=$3", [supplied.org_id, supplied.id, receipt.decision_id])).rows[0];
      if (!row) throw unavailable();
      const actor = await currentOffice(tx, supplied, sessionHash, row.unit_id);
      requireCondition(receipt.fingerprint === fingerprint, 409, "This command UUID was already used for a different gpa decision.");
      checkedGpaDecision(row); await recheckReportSession(tx, actor, sessionHash);
      return { decision: summary(row), replayed: true };
    }
    const preview = (await tx.query("SELECT * FROM gpa_previews WHERE org_id=$1 AND prepared_by=$2 AND id=$3", [supplied.org_id, supplied.id, input.previewId])).rows[0];
    ...
    const data = checkedGpaReview(preview.review_text, preview.preview_hash), freshData = review(fresh, version, data.expected.latestDecisionId, data.expected.latestDecisionNumber);
    ...
    const locked = (await tx.query("SELECT * FROM gpa_previews WHERE org_id=$1 AND prepared_by=$2 AND id=$3 AND consumed_at IS NULL AND expires_at>clock_timestamp() FOR UPDATE", [actor.org_id, actor.id, input.previewId])).rows[0];
    ...
    await tx.query("SELECT pg_advisory_xact_lock(hashtextextended($1,0))", ["gpa-decisions-quota:" + actor.org_id]);
    ...
    const envelope = gpaDecisionEnvelopeSchema.parse({ schemaVersion: 1, id, seriesId, number, supersedesId: prior?.latest_decision_id ?? null,
    ...
    const files = renderGpaDecision(envelope), stats = (await tx.query("SELECT count(*)::int AS count,COALESCE(sum(bytes),0)::text AS bytes FROM gpa_decisions WHERE org_id=$1", [actor.org_id])).rows[0];
    ...
    if (!prior) prior = (await tx.query("INSERT INTO gpa_series(id,org_id,unit_id,student_id,year_id,term_id,policy_id,created_by) VALUES($1,$2,$3,$4,$5,$6,$7,$8) RETURNING *", [seriesId, ...scopeParams(scope), actor.id])).rows[0];
    const row = (await tx.query(`INSERT INTO gpa_decisions(id,org_id,unit_id,series_id,student_id,year_id,term_id,policy_id,number,supersedes_id,
      ...
      snapshot_text,snapshot_hash,json_text,json_hash,csv_text,csv_hash,json_bytes,csv_bytes,bytes,gpa_summary)
      ...
      [id, actor.org_id, scope.unitId, seriesId, scope.studentId, scope.yearId, scope.termId, scope.policyId, number, envelope.supersedesId,
        ...
        input.reason, data.result.outcome, files.snapshotText, files.snapshotHash, files.jsonText, files.jsonHash, files.csvText, files.csvHash, files.jsonBytes, files.csvBytes, files.bytes, data.result.totals === null ? null : JSON.stringify(deriveGpaSummary(data.result))])).rows[0];
    await tx.query("UPDATE gpa_series SET latest_number=$2,latest_decision_id=$3 WHERE id=$1", [seriesId, number, id]);
    const consumed = await tx.query("UPDATE gpa_previews SET consumed_at=clock_timestamp() WHERE id=$1 AND consumed_at IS NULL AND expires_at>clock_timestamp() RETURNING id", [input.previewId]);
    if (!consumed.rows.length) throw unavailable();
    await tx.query("INSERT INTO gpa_decision_commands(org_id,actor_id,command_id,fingerprint,decision_id) VALUES($1,$2,$3,$4,$5)", [actor.org_id, actor.id, input.commandId, fingerprint, id]);
    await audit(tx, actor, "gpa.decision.retained", id, { unitId: scope.unitId, policyVersionId: version.id, previewId: input.previewId, schemaVersion: 1, snapshotHash: files.snapshotHash, jsonHash: files.jsonHash, csvHash: files.csvHash, bytes: files.bytes });
    await recheckReportSession(tx, actor, sessionHash);
...
async function historical<T>(db: Database, supplied: Actor, sessionHash: string | undefined, rawId: unknown, fn: (tx: Queryable, actor: Actor, row: Row) => Promise<T>) {
  ...
  return transaction(db, async tx => {
    const lookup = (await tx.query("SELECT unit_id FROM gpa_decisions WHERE id=$1 AND org_id=$2", [id, supplied.org_id])).rows[0];
    ...
    const row = (await tx.query("SELECT *,encode(convert_to(csv_text,'UTF8'),'base64') AS csv_base64 FROM gpa_decisions WHERE id=$1 AND org_id=$2", [id, actor.org_id])).rows[0];
    if (!row) throw unavailable();
    const result = await fn(tx, actor, row); await recheckReportSession(tx, actor, sessionHash); return result;
  });
}
export async function getGpaDecision(db: Database, supplied: Actor, sessionHash: string | undefined, id: unknown): Promise<GpaDecisionDetail> {
  return historical(db, supplied, sessionHash, id, async (_tx, _actor, row) => checkedGpaDecision(row));
}
export async function exportGpaDecision(db: Database, supplied: Actor, sessionHash: string | undefined, id: unknown, raw: unknown) {
  const { format } = gpaDecisionExportInput.parse(raw);
  return historical(db, supplied, sessionHash, id, async (tx, actor, row) => {
    checkedGpaDecision(row); const content = format === "json" ? row.json_text : Buffer.from(row.csv_base64, "base64").toString("utf8"), fileHash = format === "json" ? row.json_hash : row.csv_hash;
    await audit(tx, actor, "gpa.decision.exported", row.id, { unitId: row.unit_id, format, fileHash, bytes: Buffer.byteLength(content), schemaVersion: 1 });
    return { content: content as string, fileHash: fileHash as string, filename: "gpa-decision-" + row.id + "-v1." + format,
...
async function checkListScope(tx: Queryable, actor: Actor, input: { unitId: string; studentId: string; yearId: string; termId?: string; policyId?: string }) {
  ...
  if (input.policyId && !(await tx.query("SELECT id FROM gpa_policies WHERE org_id=$1 AND unit_id=$2 AND year_id=$3 AND id=$4", [actor.org_id, input.unitId, input.yearId, input.policyId])).rows.length) throw unavailable();
}
export async function eligibleGpaIssues(db: Database, supplied: Actor, sessionHash: string | undefined, raw: unknown): Promise<EligibleGpaIssues> {
  const input = eligibleGpaIssuesInput.parse(raw);
  return transaction(db, async tx => {
    ...
    const result = eligibleGpaIssuesSchema.parse({ items: rows.map(r => ({ cardId: r.card_id, cardVersion: r.card_version, issueId: r.issue_id, issueNumber: r.number, issueHash: r.snapshot_hash, termIds: r.term_ids, issuedAt: iso(r.issued_at) })) });
    await recheckReportSession(tx, actor, sessionHash!); return result;
  });
}
export async function listGpaDecisions(db: Database, supplied: Actor, sessionHash: string | undefined, raw: unknown): Promise<GpaDecisionList> {
  const input = gpaDecisionListInput.parse(raw); let cursor: z.infer<typeof gpaDecisionCursorSchema> | null = null;
  if (input.cursor) {
    try { const text = Buffer.from(input.cursor, "base64url").toString("utf8"); cursor = gpaDecisionCursorSchema.parse(JSON.parse(text)); requireCondition(Buffer.from(canonical(cursor)).toString("base64url") === input.cursor, 400, "Invalid decision cursor."); }
    catch { throw new Problem(400, "Invalid decision cursor."); }
  }
  return transaction(db, async tx => {
    const actor = await currentOffice(tx, supplied, sessionHash, input.unitId); await checkListScope(tx, actor, input);
    const filterHash = digest(canonical({ schemaVersion: 1, kind: "gpa_decision_filter", orgId: actor.org_id,
      unitId: input.unitId, studentId: input.studentId, yearId: input.yearId, termId: input.termId ?? null, policyId: input.policyId ?? null, latestOnly: input.latestOnly }));
    requireCondition(!cursor || cursor.filterHash === filterHash, 400, "This decision cursor belongs to different filters. Restart the list.");
    const revision = (await tx.query(`SELECT count(*)::int AS revision FROM gpa_decisions WHERE org_id=$1 AND unit_id=$2 AND student_id=$3 AND year_id=$4
    ...
    const rows = (await tx.query(`SELECT d.id,d.series_id,d.number,d.supersedes_id,d.policy_version_id,d.outcome,d.gpa_summary,d.snapshot_hash,d.json_hash,d.csv_hash,d.captured_at,
      d.student_id,d.year_id,d.term_id,d.policy_id,v.version AS policy_version,(s.latest_decision_id=d.id) AS latest
      FROM gpa_decisions d JOIN gpa_series s ON s.id=d.series_id JOIN gpa_policy_versions v ON v.id=d.policy_version_id
    ...
    const result = gpaDecisionListSchema.parse({ items: rows.slice(0, limits.page).map(r => ({ ...summary(r), studentId: r.student_id, yearId: r.year_id, termId: r.term_id,
      policyId: r.policy_id, policyVersion: r.policy_version, latest: r.latest })), nextCursor: rows.length > limits.page ? Buffer.from(canonical({ schemaVersion: 1, capturedAt: iso(last.captured_at), id: last.id, revision, filterHash })).toString("base64url") : null });
    await recheckReportSession(tx, actor, sessionHash!); return result;
  }, true);
}
async function currentMetadata(tx: Queryable, orgId: string, data: GpaReviewData): Promise<Pick<GpaDecisionCurrentness, "policyState" | "cardState">> {
  const policy = (await tx.query("SELECT archived,active_policy_version_id FROM gpa_policies WHERE org_id=$1 AND id=$2 FOR SHARE", [orgId, data.policyVersion.policyId])).rows[0];
...
export async function gpaDecisionCurrentness(db: Database, supplied: Actor, sessionHash: string | undefined, id: unknown): Promise<GpaDecisionCurrentness> {
  const detail = await getGpaDecision(db, supplied, sessionHash, id), data = detail.decision.data;
  ...
  try {
    ...
    return await transaction(db, async tx => {
      const authority = await lockStandingSourceAuthority(tx, supplied, sessionHash!, copy);
      await tx.query("SELECT id FROM gpa_policies WHERE org_id=$1 AND id=$2 FOR SHARE", [authority.orgId, data.policyVersion.policyId]);
      ...
      const checkedAt = iso((await tx.query("SELECT clock_timestamp() AS now")).rows[0].now);
      await recheckReportSession(tx, authority.actor, sessionHash!);
      return gpaDecisionCurrentnessSchema.parse({ checkedAt, state, ...metadata });
    });
  } catch (error: any) {
    ...
    return transaction(db, async tx => {
      ...
      const checkedAt = iso((await tx.query("SELECT clock_timestamp() AS now")).rows[0].now);
      await recheckReportSession(tx, actor, sessionHash!);
      return gpaDecisionCurrentnessSchema.parse({ checkedAt, ...metadata,
        state: metadata.cardState === "changed" || error.status === 409 ? "source_changed" : "source_check_unavailable" });
    });
  }
}

export function installGpaDecisions(app: Express, db: Database) {
  const base = "/api/school/gpa", actor = schoolActor, session = (req: any) => (req as AppRequest).sessionHash;
  const send = (res: any, data: unknown) => res.set("Cache-Control", "private, no-store").json(data);
  app.get(base + "/eligible-issues", async (req, res) => send(res, await eligibleGpaIssues(db, actor(req), session(req), req.query)));
  app.post(base + "/previews", async (req, res) => { const data = await prepareGpa(db, actor(req), session(req), req.body); send(res.status(201), data); });
  app.get(base + "/previews/:id", async (req, res) => send(res, await getGpaPreview(db, actor(req), session(req), req.params.id)));
  app.post(base + "/decisions", async (req, res) => { const data = await retainGpa(db, actor(req), session(req), req.body); send(res.status(data.replayed ? 200 : 201), data); });
  app.get(base + "/decisions", async (req, res) => send(res, await listGpaDecisions(db, actor(req), session(req), req.query)));
  app.get(base + "/decisions/:id", async (req, res) => send(res, await getGpaDecision(db, actor(req), session(req), req.params.id)));
  app.get(base + "/decisions/:id/currentness", async (req, res) => send(res, await gpaDecisionCurrentness(db, actor(req), session(req), req.params.id)));
  app.get(base + "/decisions/:id/export", async (req, res) => {
    const file = await exportGpaDecision(db, actor(req), session(req), req.params.id, req.query);
