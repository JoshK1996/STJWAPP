...
/** Account locks precede session proof. PIN retains its clock-only MFA exemption. */
export async function recheckClockSession(tx:Queryable,actor:Actor,sessionHash:string){
  requireCondition((await tx.query(`SELECT s.token_hash FROM sessions s WHERE s.token_hash=$1 AND s.org_id=$2 AND s.user_id=$3 AND s.mode=$4
    AND s.expires_at>clock_timestamp() AND EXISTS(SELECT 1 FROM users u WHERE u.org_id=s.org_id AND u.id=s.user_id AND u.active AND NOT u.requires_credential_change)
...
export async function currentClockActor(tx:Queryable,supplied:Actor,sessionHash:string,write:boolean):Promise<Actor>{
  ...
  const user=(await tx.query(`SELECT id,org_id,name,email,role,active,requires_credential_change FROM users WHERE id=$1 AND org_id=$2 FOR ${write?'UPDATE':'SHARE'}`,[supplied.id,supplied.org_id])).rows[0];
  ...
  await recheckClockSession(tx,actor,sessionHash);return actor;
}
