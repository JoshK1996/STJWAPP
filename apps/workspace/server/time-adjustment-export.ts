...
export function validateTimeEnvelope(raw: unknown): TimeAdjustmentEnvelope {
  ...
  requireCondition(request.status==='approved' ? value.result!==null && value.resultHash!==null : value.result===null && value.resultHash===null && request.resultShiftId===null && request.resultRevision===null,422,'Time result evidence is inconsistent.');
  ...
  requireCondition(request.status==='pending' ? request.version===1 && request.resolvedBy===null && request.resolvedAt===null && request.resolutionNote===null : request.version===2 && request.resolvedBy!==null && request.resolvedAt!==null && request.resolutionNote!==null,422,'Time decision evidence is inconsistent.');
