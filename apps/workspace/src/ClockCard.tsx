...
export default function ClockCard({state,onChange,notify,onSessionExpired,onPendingChange,large=false}:{state:any;onChange:()=>Promise<void>;notify:(s:string,error?:boolean)=>void;onSessionExpired:()=>void;onPendingChange:(pending:boolean)=>void;large?:boolean}) {
  ...
  return <section ref={cardRef} className={'clock-card clock-experience '+(large?'large ':'')+(shift?'is-active':'is-ready')+(onBreak?' is-break':'')} aria-label="My time clock">
    ...
    <div className="clock-stage">
      ...
      <div className="clock-sculpture"><div className="clock-dial-shadow" aria-hidden="true"/><div className="clock-dial">
        ...
        <div className="clock-dial-content">{onBreak?<Coffee size={25}/>:<Clock3 size={25}/>}<span>{shift?'ELAPSED SHIFT':'READY TO BEGIN'}</span><strong aria-label={shift?'Elapsed shift '+time:'Not clocked in'}>{shift?time:'00:00:00'}</strong><small>{shift?'Includes recorded breaks':'One tap to start your day'}</small></div>
      </div><div className="clock-floating-note"><span className="clock-note-icon">{shift?<Check size={17}/>:<Play size={17}/>}</span><div><strong>{shift?onBreak?'Break in progress':'Shift in progress':'Your next step'}</strong><span>{shift?segmentTime:'Choose your job below'}</span></div></div></div>
    </div>
    ...
    <div className="clock-control-deck"><label className="clock-job">{shift?'Change to another job':'I’m working as'}<select aria-label="Clock job" value={job} onChange={e=>setJob(e.target.value)} disabled={locked||onBreak}>{!state.jobs.length&&<option value="">No assigned jobs</option>}{state.jobs.map((j:any)=><option key={j.id} value={j.id}>{j.title} · {j.unit_name}</option>)}</select>{onBreak&&<small>End your break to change jobs.</small>}</label><div className="clock-actions">{!shift?<button className="button clock-start" onClick={()=>command('clock_in')} disabled={locked||!job}><Play size={21} fill="currentColor"/>Clock in<ArrowRight size={20}/></button>:<><button className={'button '+(onBreak?'clock-resume':'clock-pause')} onClick={()=>command(onBreak?'end_break':'start_break')} disabled={locked}>{onBreak?<Play size={19}/>:<Coffee size={19}/>} {onBreak?'End break':'Take a break'}</button>{job!==shift.job_id&&!onBreak&&<button className="button clock-switch" onClick={()=>command('switch_job')} disabled={locked||!job}><ArrowRightLeft size={18}/>Switch job</button>}<button className="button clock-finish" onClick={()=>command('clock_out')} disabled={locked}><LogOut size={19}/>Clock out</button></>}</div></div>
