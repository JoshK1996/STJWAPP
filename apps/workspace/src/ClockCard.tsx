...
import { PendingClockCard } from './ScheduledClock';
import './clock-experience.css';
type ClockAction='clock_in'|'clock_out'|'switch_job'|'start_break'|'end_break';
type ClockAttempt={body:Readonly<{action:ClockAction;jobId?:string;commandId:string}>;label:string;uncertain:boolean;confirmed:boolean;scheduledStart?:boolean};
...
export default function ClockCard({state:providedState,onChange,notify,onSessionExpired,onPendingChange,large=false}:{state:any;onChange:()=>Promise<void>;notify:(s:string,error?:boolean)=>void;onSessionExpired:()=>void;onPendingChange:(pending:boolean)=>void;large?:boolean}) {
  const [job,setJob]=useState(''),[now,setNow]=useState(Date.now()),[liveState,setLiveState]=useState<any>(null);
  const state=liveState&&(!providedState||Date.parse(liveState.serverTime)>Date.parse(providedState.serverTime))?liveState:providedState;
  const pendingStart=state?.preclock?.pending;
  const clockLabel=(instant:string)=>new Intl.DateTimeFormat(undefined,{timeZone:state?.preclock?.timezone??'America/New_York',hour:'numeric',minute:'2-digit'}).format(new Date(instant));
  ...
  useEffect(()=>{if(!locked&&state?.jobs?.length)setJob(state.shift?.job_id??state.preclock?.pending?.jobId??state.jobs[0].id);},[state?.shift?.job_id,state?.preclock?.pending?.jobId,state?.jobs?.length,locked]);
  useEffect(()=>{
  ...
  async function refreshSaved(attempt:ClockAttempt){
    ...
    try{
      ...
      release();notify(attempt.scheduledStart?'Your scheduled start is saved. Start work at the time shown.':successMessages[attempt.body.action]);
    }catch(error){
      if(!mounted.current||attemptRef.current!==attempt)return;
  ...
  async function send(attempt:ClockAttempt){
    ...
    try{
      const saved=await api('/clock',attempt.body);
      attempt.scheduledStart=!!saved.preclock?.pending&&!saved.shift;
      if(!mounted.current||attemptRef.current!==attempt)return;
  ...
  function command(action:ClockAction){
    if(busyRef.current||attemptRef.current||(action==='clock_in'&&pendingStart))return;
  ...
  useEffect(()=>{
    ...
    const poll=async()=>{
      if(stopped||inflight||busyRef.current||attemptRef.current||document.hidden)return;inflight=true;
      try{const fresh=await api('/clock',undefined,'GET',controller.signal);if(stopped||!mounted.current)return;setLiveState(fresh);if(!fresh.preclock?.pending)void onChange().catch(()=>{});}
      catch(error){if(!stopped&&error instanceof ApiError&&[401,403].includes(error.status))expired();}
      finally{inflight=false;}
    };
    const timer=setInterval(()=>void poll(),2000);const visible=()=>{if(!document.hidden)void poll();};document.addEventListener('visibilitychange',visible);
    return()=>{stopped=true;clearInterval(timer);controller.abort();document.removeEventListener('visibilitychange',visible);};
  },[pendingStart?.id,onChange]);
  ...
  const selectedSchedule=state.preclock?.schedules?.find((row:any)=>row.jobId===job);
  const early=state.preclock?.policy?.noEarlyClockIn&&selectedSchedule?.phase==='upcoming';
  const scheduleMissing=state.preclock?.policy?.noEarlyClockIn&&!selectedSchedule;
  const pendingTime=pendingStart?clockLabel(pendingStart.startsAt):'';
  ...
  const status=locked?(phase==='uncertain'?'Confirmation needed':attemptRef.current?.confirmed?'Saved · refresh pending':'Saving…'):shift?onBreak?'On a break':'On the clock':pendingStart?'Pending start':'Clocked out';
  return <section ref={cardRef} className={'clock-card clock-experience '+(large?'large ':'')+(shift?'is-active':'is-ready')+(onBreak?' is-break':'')} aria-label="My time clock">
    ...
    <div className="clock-quick-summary">
      ...
      <div className="clock-elapsed"><span>{shift?'Elapsed shift':pendingStart?'Scheduled start':'Ready to begin'}</span><strong aria-label={shift?'Elapsed shift '+time:'Not clocked in'}>{shift?time:pendingStart?pendingTime:'00:00:00'}</strong><small>{shift?'Includes breaks':pendingStart?'Not clocked in yet':'Choose your job below'}</small></div>
    </div>

    {!shift&&pendingStart?<PendingClockCard preclock={state.preclock} busy={locked} onPendingChange={onPendingChange} onChanged={fresh=>{setLiveState(fresh);void onChange().catch(error=>notify((error as Error).message,true));}} notify={notify}/>:<div className="clock-control-deck"><label className="clock-job">{shift?'Change job':'I’m working as'}<select aria-label="Clock job" value={job} onChange={e=>setJob(e.target.value)} disabled={locked||onBreak}>{!state.jobs.length&&<option value="">No assigned jobs</option>}{state.jobs.map((j:any)=><option key={j.id} value={j.id}>{j.title} · {j.unit_name}</option>)}</select>{shift&&job!==shift.job_id&&!onBreak&&<small className="clock-next-job">Selected: <strong>{selected?.title}</strong>{selected?.unit_name&&<> · {selected.unit_name}</>}</small>}{onBreak&&<small>End your break to change jobs.</small>}</label><div className="clock-actions">{!shift?<button className="button clock-start" onClick={()=>command('clock_in')} disabled={locked||!job||scheduleMissing}><Play size={21} fill="currentColor"/>{early?'Start at '+clockLabel(selectedSchedule.startsAt):scheduleMissing?'No scheduled shift':'Clock in'}<ArrowRight size={20}/></button>:<><button className="button clock-finish" onClick={()=>command('clock_out')} disabled={locked}><LogOut size={19}/>Clock out</button><button className={'button '+(onBreak?'clock-resume':'clock-pause')} onClick={()=>command(onBreak?'end_break':'start_break')} disabled={locked}>{onBreak?<Play size={19}/>:<Coffee size={19}/>} {onBreak?'End break':'Take a break'}</button>{job!==shift.job_id&&!onBreak&&<button className="button clock-switch" onClick={()=>command('switch_job')} disabled={locked||!job}><ArrowRightLeft size={18}/>Switch job</button>}</>}</div></div>}
    {!shift&&!pendingStart&&state.preclock?.latest?.status==='blocked'&&<PendingClockCard preclock={state.preclock} onChanged={()=>{}} notify={notify}/>}
    {!shift&&!pendingStart&&state.preclock?.policy?.noEarlyClockIn&&<p className="clock-experience-help">{early?'Your time will start at '+clockLabel(selectedSchedule.startsAt)+'. Start work then.':scheduleMissing?'No matching assigned shift is scheduled today. Ask a manager to update your schedule or clock setting.':'Your assigned shift has started. Clock in when you begin working.'}</p>}
    ...
    <div className="clock-stage">
      ...
      <div className="clock-sculpture" aria-hidden="true"><div className="clock-dial-shadow" aria-hidden="true"/><div className="clock-dial">
        ...
        <div className="clock-dial-content">{onBreak?<Coffee size={25}/>:<Clock3 size={25}/>}<span>{shift?'ELAPSED SHIFT':pendingStart?'PENDING START':'READY TO BEGIN'}</span><strong>{shift?time:pendingStart?pendingTime:'00:00:00'}</strong><small>{shift?'Includes recorded breaks':pendingStart?'Start work then':'One tap to start your day'}</small></div>
      </div><div className="clock-floating-note"><span className="clock-note-icon">{shift||pendingStart?<Check size={17}/>:<Play size={17}/>}</span><div><strong>{shift?onBreak?'Break in progress':'Shift in progress':pendingStart?'Pending start saved':'Your next step'}</strong><span>{shift?segmentTime:pendingStart?'Start work at '+pendingTime:'Choose your job above'}</span></div></div></div>
    </div>
