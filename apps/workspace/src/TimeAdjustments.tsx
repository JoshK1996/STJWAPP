...
import { AdjustmentTimeField, TimeEvidence, resolveEnteredTime, snapshotTotals, timeLabel } from "./TimeAdjustmentEvidence";
import "./time-adjustments.css";

export type AdjustmentTarget = { kind: "missing_shift" } | { kind: "close_open_shift"; shiftId: string } | { kind: "request"; requestId: string } | { kind: "shift_requests"; shiftId: string };
...
type ProposalReview = { body: Record<string, unknown>; snapshot: TimeSnapshot };
const blankRow = (): Row => ({ key: crypto.randomUUID(), jobId: "", kind: "work", start: "", end: "", startOffset: null, endOffset: null });
const kindLabel = (kind: string) => kind === "missing_shift" ? "Missing shift" : "Reviewed clock-out";
const denied = (error: unknown) => error instanceof ApiError && [401, 403, 404].includes(error.status);
export default function TimeAdjustments({ me, view, onView, target, onTargetConsumed, onChanged, onDirty, notify }: {
  me: any; view: View; onView(view: View): void; target: AdjustmentTarget | null; onTargetConsumed(): void;
  onChanged(): Promise<void>; onDirty(dirty: boolean): void; notify(message: string, error?: boolean): void;
}) {
  ...
  function mayLeave() { return !busyRef.current && (!dirty || window.confirm(uncertain ? "The last request may already be saved. Discard this retry? Check review requests before submitting another." : "Discard this unsaved time adjustment?")); }
  function closeCompose() {
    if (!mayLeave()) return;
    sourceEpoch.current++; optionsEpoch.current++;
    setCompose(null); setSource(null); setOptions(null); setStaff([]); setReason(""); setProposedEnd(""); setRows([blankRow()]); setProposalReview(null); attemptRef.current = null; setUncertain(false); setError(""); restoreFocus();
  }
  function closeDetail() {
    if (!mayLeave()) return;
    detailEpoch.current++;
    setDetail(null); setHistory(null); setDecision(null); setNote(""); attemptRef.current = null; setUncertain(false); setError(""); restoreFocus();
  }
  ...
  async function missing() {
    if (!mayLeave()) return; rememberOpener(); setAccessLost(false); setCompose("missing"); setDetail(null); setHistory(null); setRows([blankRow()]); setReason(""); setEmployeeId(currentId); setSource(null); setProposalReview(null); setError("");
    const lifetime = epoch.current;
    await Promise.all([loadOptions(currentId), me.permissions.manage ? api<{ rows: any[] }>("/staff").then(result => { if (lifetime === epoch.current) setStaff(result.rows); }).catch(error => { if (lifetime === epoch.current) fail(error); }) : Promise.resolve()]);
  }
  async function closure(shiftId: string, preserve = false) {
    ...
    try {
      ...
      if (lifetime === epoch.current && marker === sourceEpoch.current) { setSource(result); setCompose("closure"); setDetail(null); setHistory(null); }
  ...
  useEffect(() => {
    ...
    if (target.kind === "missing_shift") void missing();
  ...
  function prepare(event: FormEvent) {
    ...
    try {
      ...
      setProposalReview({ body, snapshot });
  ...
  async function send(attempt?: Attempt) {
    ...
    try {
      ...
      try { await onChanged(); } catch { notify("The request was saved. Refresh the workspace to update current clock and report views.", true); }
    } catch (error) {
      if (lifetime !== epoch.current) return;
  ...
  function submitProposal() { if (!proposalReview) return; void send({ path: "/time-adjustments", body: { ...proposalReview.body, commandId: crypto.randomUUID() }, message: "Proposal submitted. Recorded time changes only after independent approval." }); }
  ...
  return <div className="time-adjustments">
    <div className="adjustment-navigation" aria-label="Time record views">
      ...
      <button type="button" className="button secondary" disabled={busy || loading} onClick={() => void missing()}><Plus size={16} />Report a missing shift</button>
    </div>
    ...
    {view !== "records" && <>
      <div className="adjustment-filters">
        {view === "requests" && <>{!shiftFilter && <><label>Work from<input type="date" value={start} onChange={event => setStart(event.target.value)} /></label><label>Work through<input type="date" value={end} onChange={event => setEnd(event.target.value)} /></label></>}
          ...
          <label>Status<select value={status} onChange={event => setStatus(event.target.value)}><option value="">All statuses</option>{['pending', 'approved', 'declined', 'cancelled'].map(value => <option key={value} value={value}>{value}</option>)}</select></label></>}
      ...
      <Panel title={view === "open" ? "Currently open shifts" : "Independent review requests"} detail={view === "open" ? `All start dates · ${zone}. This is a current observation, not a saved historical source.` : shiftFilter ? `All dates for this shift · ${zone}. Submitting does not change recorded time.` : `Proposed work overlaps these inclusive dates in ${zone}. Submitting does not change recorded time.`}>
        {view === "open" ? <div className="adjustment-cards">{openList?.items.map(row => <article key={row.shiftId}><div><strong>{row.employee.name}</strong><p>Started {timeLabel(row.startedAt, zone)}</p><Badge>Revision {row.revision}</Badge>{row.pendingRequests > 0 && <Badge tone="warm">{row.pendingRequests} pending requests</Badge>}</div><div className="adjustment-card-actions"><button type="button" className="button secondary" disabled={loading} onClick={() => void closure(row.shiftId)}>Review current source</button>{row.pendingRequests > 0 && <button type="button" className="text-link" onClick={() => { setShiftFilter(row.shiftId); onView("requests"); }}>View requests</button>}</div></article>)}</div>
          : <div className="adjustment-cards">{list?.items.map(row => <button type="button" className="adjustment-request-card" key={row.id} disabled={loading} onClick={() => void openDetail(row.id)}><div><strong>{row.employee.name}</strong><p>{kindLabel(row.kind)} · Proposed by {row.proposedBy.name}</p><p>{timeLabel(row.startedAt, zone)} to {timeLabel(row.endedAt, zone)}</p><small>Submitted {timeLabel(row.createdAt, zone)}</small></div><Badge tone={row.status === "approved" ? "green" : row.status === "pending" ? "warm" : "neutral"}>{row.status}</Badge></button>)}</div>}
    ...
    {compose && <Modal title={compose === "missing" ? "Report a missing shift" : "Request reviewed clock-out"} onClose={closeCompose}>
      {error && <p className="error" role="alert">{error}</p>}{retry}
      {proposalReview ? <div className="community-form"><h3 id="time-proposal-review-heading" tabIndex={-1}>Review this proposal</h3><p>Recorded time remains unchanged until a different authorized manager approves.</p>
        ...
        <div className="dialog-actions"><button type="button" className="button secondary" disabled={frozen} onClick={() => setProposalReview(null)}><ArrowLeft size={16} />Edit proposal</button><button type="button" className="button primary" disabled={frozen} onClick={submitProposal}>{busy ? "Submitting…" : "Submit for independent review"}</button></div>
        ...
        {compose === "missing" ? <>
          {me.permissions.manage ? <label>Employee<select aria-label="Missing shift employee" disabled={frozen || loading} value={employeeId} onChange={event => { setEmployeeId(event.target.value); setRows([blankRow()]); void loadOptions(event.target.value); }}>
          ...
          {source && <><TimeEvidence snapshot={source.source} title="Captured current source" zone={source.timezone} /><p className="panel-note">The clock continues running until a different authorized manager approves. The employee can still clock out normally.</p>
            </>}
          ...
          <AdjustmentTimeField label="Proposed clock-out" value={proposedEnd} offset={endOffset} zone={source?.timezone ?? zone} disabled={frozen} onChange={(value, offset) => { setProposedEnd(value); setEndOffset(offset); }} />
        </>}
        ...
        <div className="dialog-actions"><button type="button" className="button secondary" disabled={busy} onClick={closeCompose}>Cancel</button><button className="button primary" disabled={frozen || loading || (compose === "missing" ? !options?.allowedActions.proposeMissing || !options.jobs.length : !source?.allowedActions.propose)}>Review proposal</button></div>
      </form>}
    </Modal>}
    {detail && !compose && <Modal title={`${kindLabel(detail.request.kind)} request`} onClose={closeDetail}>
      <div className="community-form"><div className="adjustment-heading"><strong>{detail.request.employee.name}</strong><Badge tone={detail.request.status === "pending" ? "warm" : "neutral"}>{detail.request.status}</Badge></div>
        {error && <p className="error" role="alert">{error}</p>}{retry}
        <p>Proposed by {detail.request.proposedBy.name} · {timeLabel(detail.request.createdAt, zone)}</p><p className="adjustment-reason">{detail.request.reason}</p>
        ...
        {detail.result && <TimeEvidence snapshot={detail.result} title="Recorded approval result" zone={zone} />}
