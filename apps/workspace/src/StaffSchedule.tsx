...
import { api } from "./api";
import { Badge, Empty, Modal, Panel, shortName } from "./components";
...
export default function StaffSchedule({ me, staff, jobs, rows, week, zone, onWeek, onChanged, notify, onDirty, onRequests }: {
  ...
  const [showCancelled, setShowCancelled] = useState(true), [unitId, setUnitId] = useState("");
  ...
  const visible = rows.filter(row => (showCancelled || row.status !== "cancelled") && (!unitId || row.unit_id === unitId));
  const units = Array.from(new Map(rows.map(row => [row.unit_id, row.unit_name])).entries());
  ...
  async function refreshList() {
    ...
    setRefreshing(true);
    try { await onChanged(); }
    catch (cause) { if (editor) setError(errorMessage(cause)); else notify(errorMessage(cause), true); }
  ...
  async function saveShift() {
    if (!review || !editor || busy || stale) return;
    ...
    try {
      ...
      try { await onChanged(); } catch (cause) { notify(`Your change was saved, but the list could not refresh: ${errorMessage(cause)}`, true); }
    } catch (cause) { setError(errorMessage(cause)); }
  ...
  async function loadHistory(shift: Shift, before?: number) {
    ...
    } catch (cause) { if (request === historyRequest.current) setHistoryError(errorMessage(cause)); }
  ...
  return <>
    <Panel title={me.permissions.report ? "Team schedule" : "My schedule"} detail={`All dates and times use ${zone}.`} className="staff-schedule"
      ...
      <div className="staff-schedule-toolbar">
        ...
        {me.permissions.report && <label>Community<select value={unitId} onChange={event => setUnitId(event.target.value)}><option value="">All permitted communities</option>{units.map(([id, name]) => <option key={id} value={id}>{name}</option>)}</select></label>}
      ...
      <div className="calendar-grid staff-schedule-grid">
        {Array.from({ length: 7 }, (_, index) => {
          const date = DateTime.fromISO(week, { zone }).plus({ days: index }), end = date.plus({ days: 1 });
          ...
          return <section key={date.toISODate()} className={date.hasSame(DateTime.now().setZone(zone), "day") ? "today" : ""} aria-label={date.toFormat("cccc, LLLL d")}>
            <header><span>{date.toFormat("ccc")}</span><strong>{date.day}</strong></header>
            {items.map(shift => <article key={shift.id} className={`schedule-card staff-shift-card ${shift.status === "cancelled" ? "staff-shift-cancelled" : ""}`}>
              <strong>{shortName(shift.employee_name)}</strong>
...
function ShiftSnapshot({ snapshot, zone }: { snapshot: Record<string, any>; zone: string }) {
  ...
  return <dl className="staff-shift-summary"><dt>Employee</dt><dd>{snapshot.employee_name ?? snapshot.employeeName ?? snapshot.user_id ?? snapshot.userId ?? "Unavailable"}</dd>
    <dt>Job</dt><dd>{snapshot.job_title ?? snapshot.jobTitle ?? snapshot.job_id ?? snapshot.jobId ?? "Unavailable"}</dd>
    <dt>Community</dt><dd>{snapshot.unit_name ?? snapshot.unitName ?? snapshot.unit_id ?? snapshot.unitId ?? "Unavailable"}</dd>
