...
export default function WorkbookImport({ disabled, kind = 'finance', unitId, assignmentId, userId, jobId, beforeReplace, onClear, onPending, onApply, onAccessDenied }: Props) {
  ...
  const scope = kind === 'finance' || staff ? { kind } : compensation ? { kind, userId, jobId } : grade ? { kind, assignmentId } : { kind, unitId };
  ...
  async function template() {
    ...
    try {
      ...
      } else await download('/import-workbooks/template?' + new URLSearchParams({ kind, ...(kind === 'finance' || staff ? {} : grade ? { assignmentId: assignmentId ?? '' } : { unitId: unitId ?? '' }) }), grade ? 'stjw-assignment-scores.xlsx' : staff ? 'stjw-staff-template.xlsx' : schoolKind ? `stjw-${schoolKind}-template.xlsx` : 'financial-report-template.xlsx');
    }
  ...
  return <section className="workbook-import" aria-label="Excel workbook import">
    <div className="workbook-heading"><FileSpreadsheet size={23} aria-hidden="true" /><div><h3>{compensation ? 'Excel · Employee pay rates' : grade ? 'Excel · Assignment scores' : staff ? 'Excel · New staff accounts' : school ? `Excel · ${school.title}` : 'Bring in an Excel report'}</h3>
      <p>{compensation ? 'Download the current employee/job template. Keep every rate ID, including voided entries, and the record version. Keep all eleven headers; their order may change. Up to 200 rate rows.' : grade ? 'Download this assignment’s current roster. Keep every identity and version as text; edit only status, points and note. Up to 200 student rows.' : staff ? 'Use the blank text-formatted template for new accounts. Keep name, email, role, unitIds and jobIds in that order. Up to 500 staff rows.' : school ? 'Use this import type’s text-formatted template. Keep numbers, IDs, dates and permissions as exact text. Up to 500 data rows.' : 'Use our text-formatted template to preserve line codes and exact amounts. Up to 500 data rows.'}</p></div></div>
    ...
    {converted && <div className="workbook-review">
      <h4>{converted.rowCount} {compensation ? 'pay rate' : grade ? 'score' : staff ? 'staff' : school ? 'school import' : 'report'} rows from {converted.sheetName}</h4>
