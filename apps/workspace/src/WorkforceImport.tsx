...
export default function WorkforceImport({ kind, onChange, notify, onDirty, isSessionCurrent, onSessionExpired }: {
  ...
  return <section className="workforce-import" aria-label={`Bulk ${kind} import`}>
    ...
    <p>Use exact community and job names from People &amp; jobs. {kind === 'jobs' ? 'A job title must be new within its community. Assign the new jobs to employees after import.' : 'Use each employee’s email and an active job already assigned to them. Overlapping shifts are blocked.'}</p>
