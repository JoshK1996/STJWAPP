...
type Props = { me: any; board: any[]; boardState?:TeamBoardState; onNavigate: (target: Target) => void; onRefresh?:()=>void; onNavigateRecords?:(target?:TimeRecordsTarget)=>void };
...
function PersonOverview({period,name,zone,onExact,onCards}:{period:AllowancePeriod;name:string;zone:string;onExact:()=>void;onCards?:()=>void}){
...
export default function WorkforceDashboard({me,board,boardState,onNavigate,onRefresh,onNavigateRecords}:Props){
  ...
  return <section className="workforce-dashboard wf-simple" aria-label="Workforce dashboard"><header className="wf-simple-heading"><div><h1 id="workforce-heading">Team & hours</h1><p>{me.organization.name} · {unit?(me.units??[]).find((item:any)=>item.id===unit)?.name:'All permitted communities'}</p></div><button type="button" className="wf-primary-payroll" onClick={()=>onNavigate('payroll')}><Wallet size={20}/>Review payroll<ArrowRight size={17}/></button></header>
    ...
    {activePerson&&<Modal title={activePerson.name+' · hours'} onClose={()=>setPerson(null)}>{personReport.error?<p role="alert">{personReport.error}</p>:!personReport.data?<p role="status">Loading this employee’s hours…</p>:<PersonOverview name={activePerson.name} period={personReport.data.selected} zone={zone} onExact={exactPerson} onCards={onNavigateRecords?()=>{if(!activePerson||!selectedPeriod)return;onNavigateRecords({userId:activePerson.id,start:selectedPeriod.query.start,end:selectedPeriod.query.end,...(unit?{unitId:unit}:{})});}:undefined}/>}</Modal>}
  </section>;
}
