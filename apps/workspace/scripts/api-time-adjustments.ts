...
query(add(base + '/options', 'get', 'Read current employee and job choices',
  'Requires an explicit employeeId. Current assigned active jobs are offered for a missing shift. Employees/finance propose for themselves; managers may propose for permitted employees, including inactive employees with valid current assignments. No job or time is inferred. allowedActions.applyDirect identifies current administrator/owner/developer authority to save another employee immediately.', 'TimeAdjustmentOptions'), time.timeAdjustmentOptionsInput);
query(add(base + '/closure-source', 'get', 'Read the exact active source for a proposed closure',
  'Requires shiftId. Returns the complete active revision, immutable source hash, exact UTC source instants and current observation time. The recorded null end stays null and completed totals stay unknown. No clock action occurs. allowedActions.applyDirect separately identifies immediate administrator authority for another employee.', 'TimeAdjustmentSource'), time.timeAdjustmentSourceInput);
...
query(add(base, 'get', 'List missing-shift and closure review requests',
  'Supply both inclusive organization-calendar start/end dates (at most367 days), or an exact sourceShiftId matching either the original source or resulting saved card. Date filters mean proposed work overlaps the range, not submission date. Optional employee/kind/status filters. Fifty rows per page. A changed scoped revision returns409 and requires discarding accumulated pages.', 'TimeAdjustmentList'), time.timeAdjustmentListInput);
...
const direct = add(base + '/direct', 'post', 'Save another employee missing shift or missed clock-out immediately',
  'Current administrator, owner or developer for another employee only. Uses the same explicit segments or locked closure sourceHash, reason and command UUID validation as proposals. Direct application publishes a terminal applied version1 with original/proposed/result evidence, history, audit and command receipt in one transaction. Self and scoped-manager writes retain independent review. Revalidate current session, exact source, assigned jobs and overlaps. An identical command retry returns the captured result even after later time changes; a changed body cannot reuse it. This is direct administrative application, not independent approval.', 'TimeAdjustmentReceipt', time.proposeTimeAdjustmentInput, 201);
direct.responses[200] = { description: 'Exact successful direct command replay', headers: privateHeaders, content: json(ref('TimeAdjustmentReceipt')) };
