...
| --- | --- | --- | --- |
| Implemented and live; local and public hosted verification passed | Searchable quick navigator and visible workflow shortcuts | Find Payroll, time records, messages, calendar, and permitted school tools without learning the sidebar | Password/PIN and role visibility preserved; keyboard/touch operation; Escape and focus return; unsaved-change guard still applies; no hidden submit action |
| Implemented and live; local and public hosted verification passed | Consistent labeled status indicators, color accents, layered surfaces, and restrained interaction feedback | See what is working, pending, unavailable, or needs attention without reading an entire table | Icons/labels accompany colors; zero/unknown/loading remain distinct; artwork/depth/reduced-motion preferences honored; data geometry unchanged |
| Implemented and live; local and public hosted verification passed | Mobile layout and long-content audit | Complete workflows on small screens without clipped controls, vertical letter stacks, or page overflow | Record checked pages, widths/themes, actual interactions and remaining issues; do not claim every phone is verified from one screenshot |
| Implemented for Payroll; release evidence in STATUS | Personal saved report filters and a clear reset | Reopen a useful employee/community/date grouping without repetitive setup | Persist filter definitions, not private result rows; reauthorize every fetch; explain moving dates such as “This week”; handle deleted/inaccessible units |
| Implemented for recorded hours; release evidence in STATUS | Payroll preparation checklist and comparison view | Surface ongoing segments, missing configuration, and period-to-period differences in one place | Show evidence and scope; never label hours “approved payroll” without a real approval/closeout model; previous periods use identical filters and equal calendar-day counts; incomplete periods and DST duration differences are explicit; corrections still require Time records review |
...
| Implemented; release evidence in STATUS; physical-device acceptance remains | Installable web-app entry point and guarded update prompt | Open STJW directly from a device's app launcher and explicitly reload a deployed update | Manifest/icons/start route, normal sessions, guarded updates and synthetic browser checks; no offline writes or private cache; native phone installation still needs device checks |
...
STJW now implements the manifest, actual school-crest icons, capability-based install guidance and explicit guarded updates described in [INSTALLATION](INSTALLATION.md) and [ADR 0003](adr/0003-installation-and-safe-updates.md). Chrome's documented install-promotion criteria include HTTPS, a web-app manifest with names/icons/start URL/display, and engagement conditions. Installation behavior differs among browsers; the manifest alone is not evidence that the app works offline or supports every mobile feature. [Chrome team installability guidance](https://web.dev/articles/install-criteria)
