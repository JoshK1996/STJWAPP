...
Web production uses only `stjw_runtime`. It verifies schema/privileges and never migrates or seeds. Review and stage an exact manifest; run separate database maintenance as a dry run, then commit; deploy that exact source. Migrations 001-047 are deployed and immutable. See `docs/DATABASE-OPERATIONS.md`.
